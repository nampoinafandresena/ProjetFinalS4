...
<body>
                    ...
                    <div class="space-y-5">
                        ...
                        <div id="fraisOptionField" style="display:none;">
                            <div class="flex items-center gap-3 p-3 rounded-xl bg-slate-50 border border-slate-200">
                                <input type="checkbox" name="inclure_frais" id="inclureFrais" value="1" checked onchange="calculerFrais()" class="w-5 h-5 text-emerald-600 border-slate-300 rounded focus:ring-emerald-500" />
                                <label for="inclureFrais" class="text-sm font-medium text-slate-700 cursor-pointer">
                                    Inclure les frais dans le montant total
                                </label>
                            </div>
                            <p class="text-xs text-slate-400 mt-1 ml-9">
                                Si coché, les frais seront ajoutés au montant retiré.
                            </p>
                        </div>
                        ...
                        <div class="p-3 rounded-xl bg-amber-50 border border-amber-200" id="fraisDisplay">
                            ...
                            <div class="flex justify-between text-sm mt-2 pt-2 border-t border-amber-200" id="totalDisplay" style="display:none;">
                                <span class="text-amber-700">Montant total débité :</span>
                                <span class="font-bold text-amber-800" id="totalText">0 Ar</span>
                            </div>
                        </div>
    ...
    <script>
        ...
        function openModal(type) {
            ...
            const fraisOptionField = document.getElementById('fraisOptionField');
            ...
            if (type === 'depot') {
                ...
                fraisOptionField.style.display = 'none';
                totalDisplay.style.display = 'none';
                document.getElementById('fraisText').textContent = '0 Ar';
                document.getElementById('inclureFrais').checked = false;
            } else if (type === 'retrait') {
                title.textContent = 'Retrait';
                recipient.style.display = 'none';
                form.action = retraitUrl;
                fraisDisplay.style.display = 'block';
                fraisOptionField.style.display = 'block';
                totalDisplay.style.display = 'block';
                document.getElementById('inclureFrais').checked = true;
            } else {
            ...
            document.getElementById('fraisText').textContent = '0 Ar';
            document.getElementById('totalText').textContent = '0 Ar';
        }
                    ...
                    .then(data => {
                        ...
                        if (data.success) {
                            ...
                            if (currentOperation === 'retrait') {
                                frais = data.frais_retrait;
                                label = 'Frais (Retrait) :';
                            } else {
                                frais = data.frais_transfert;
                                label = 'Frais (Transfert) :';
                            }

                            document.getElementById('fraisLabel').textContent = label;

                            if (inclureFrais && currentOperation === 'retrait') {
                                // Si les frais sont inclus, le montant total = montant + frais
                                const montantTotal = parseFloat(montant) + frais;
                                document.getElementById('fraisText').textContent = numberFormat(frais) + ' Ar';
                                document.getElementById('totalText').textContent = numberFormat(montantTotal) + ' Ar';
                                document.getElementById('totalDisplay').style.display = 'block';
                            } else {
                                document.getElementById('fraisText').textContent = numberFormat(frais) + ' Ar';
                                if (currentOperation === 'retrait') {
                                    document.getElementById('totalText').textContent = numberFormat(parseFloat(montant)) + ' Ar';
                                    document.getElementById('totalDisplay').style.display = 'block';
                                } else {
