...
<body>
    ...
    <header class="bg-white/80 backdrop-blur-xl border-b border-slate-100 sticky top-0 z-30">
        <div class="max-w-5xl mx-auto px-4 sm:px-6 py-4 flex items-center justify-between">
            <div class="flex items-center gap-3">
                <div
                    class="w-10 h-10 rounded-2xl bg-gradient-to-br from-emerald-500 to-teal-600 flex items-center justify-center shadow-lg shadow-emerald-500/20">
    ...
    <main class="max-w-5xl mx-auto px-4 sm:px-6 py-8 space-y-6">
        <!-- Balance Card -->
        <div
            class="relative overflow-hidden rounded-3xl bg-gradient-to-br from-slate-900 via-slate-800 to-emerald-900 p-8 text-white shadow-2xl shadow-slate-900/20">
            ...
            <div class="relative">
                ...
                <p class="text-5xl font-bold mt-2 tracking-tight"><?= number_format($user['solde'], 0, ',', ' ') ?>
                    <span class="text-xl font-normal text-emerald-200/60">Ar</span>
                </p>
        ...
        <div class="bg-white rounded-3xl border border-slate-100 overflow-hidden">
            <div class="flex items-center justify-between px-6 py-5 border-b border-slate-100">
                ...
                <a href="<?= base_url('client/history') ?>"
                    class="text-sm font-semibold text-emerald-600 hover:text-emerald-700 transition-colors">Voir
                    tout</a>
            </div>
                ...
                <?php if (!empty($recent_transactions)): ?>
                    ...
                    <?php foreach ($uniqueTransactions as $tx): ?>
                        ...
                        <div class="flex items-center gap-4 px-6 py-4 hover:bg-slate-50/50 transition-colors">
                            <div
                                class="w-10 h-10 rounded-xl <?= $isDepot ? 'bg-emerald-50 text-emerald-600' : ($isTransfert ? 'bg-sky-50 text-sky-600' : 'bg-amber-50 text-amber-600') ?> flex items-center justify-center flex-shrink-0">
                                <?php if ($isDepot): ?>
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <p class="text-xs text-slate-500"><?= date('d/m/Y H:i', strtotime($tx['date_transaction'])) ?>
                                </p>
                            </div>
                            <div class="text-right">
                                ...
                                <?php if (isset($tx['frais_appliques']) && $tx['frais_appliques'] > 0): ?>
                                    <p class="text-xs text-slate-400">Frais:
                ...
                <form id="operationForm" method="post">
                    ...
                    <div class="space-y-5">
                        ...
                        <div id="recipientField" style="display:none;">
                            <label class="block text-sm font-semibold text-slate-700 mb-2">Numéro du
                                destinataire</label>
                            <input type="tel" name="destinataire" id="destinataire" placeholder="0331234567"
                                class="input-field" />
                        </div>

                        <div>
                            <label class="block text-sm font-semibold text-slate-700 mb-2">Montant (Ar)</label>
                            <input type="number" name="montant" id="montantInput" placeholder="0" min="0" step="100"
                                class="input-field text-lg font-bold" oninput="calculerFrais()" />
                        </div>

                        <div class="p-3 rounded-xl bg-amber-50 border border-amber-200" id="fraisDisplay">
                            <div class="flex justify-between text-sm">
                                <span class="text-amber-700">Frais estimés :</span>
                        ...
                        <div class="flex gap-3 pt-2">
                            <button type="button" onclick="closeModal()"
                                class="flex-1 py-3 rounded-xl border border-slate-200 text-slate-700 font-semibold hover:bg-slate-50 transition-all">Annuler</button>
                            <button type="submit"
                                class="flex-1 py-3 rounded-xl bg-emerald-600 text-white font-semibold hover:bg-emerald-700 transition-all">Confirmer</button>
                        </div>
                    </div>
                </form>
            </div>
    ...
    <script>
        ...
        function calculerFrais() {
            ...
            fraisTimeout = setTimeout(function () {
                fetch('<?= base_url('client/calculer-frais') ?>', {
                    method: 'POST',
                    headers: {
                    ...
                    .then(data => {
                        document.getElementById('fraisLoading').style.display = 'none';
                        if (data.success) {
                            document.getElementById('fraisText').textContent = data.frais_formate;
                        } else {
                            document.getElementById('fraisText').textContent = '0 Ar';
                        }
                    })
