...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .op-header { padding: 20px 24px; border-bottom: 1px solid #f1f5f9; display: flex; justify-content: space-between; align-items: center; }
        .bracket-item { padding: 16px 24px; border-bottom: 1px solid #f8fafc; display: flex; align-items: center; gap: 16px; transition: background 0.2s; }
        ...
        .modal-content { background: white; border-radius: 24px; max-width: 500px; width: 100%; padding: 24px; animation: scaleIn 0.2s ease-out; }
        ...
        .alert-success { background: #d1fae5; color: #065f46; padding: 12px 16px; border-radius: 12px; border: 1px solid #a7f3d0; }
        .alert-error { background: #fee2e2; color: #991b1b; padding: 12px 16px; border-radius: 12px; border: 1px solid #fca5a5; }
    </style>
</head>
<body>
    <!-- Sidebar -->
    <aside class="sidebar">
        ...
        <nav class="flex-1 space-y-1">
            <button class="nav-item" onclick="window.location.href='operator-dashboard.html'">Dashboard</button>
            <button class="nav-item" onclick="window.location.href='operator-prefixes.html'">Préfixes</button>
            <button class="nav-item active">Opérations & Frais</button>
            <button class="nav-item" onclick="window.location.href='operator-gains.html'">Gains</button>
            <button class="nav-item" onclick="window.location.href='operator-clients.html'">Clients</button>
        </nav>
    ...
    <nav class="lg:hidden mobile-nav">
        <button class="mobile-nav-item" onclick="window.location.href='operator-dashboard.html'">Dashboard</button>
        <button class="mobile-nav-item" onclick="window.location.href='operator-prefixes.html'">Préfixes</button>
        <button class="mobile-nav-item active">Frais</button>
        <button class="mobile-nav-item" onclick="window.location.href='operator-gains.html'">Gains</button>
        <button class="mobile-nav-item" onclick="window.location.href='operator-clients.html'">Clients</button>
    </nav>
                    ...
                    if ($bareme['id_type_operation'] == $retraitId) {
                        ...
                        <div class="bracket-item">
                            <div class="flex-1">
                                ...
                                <p class="text-xs text-slate-500"><?= esc($bareme['operateur']) ?></p>
                            </div>
                            <div class="text-right">
                                <p class="font-semibold text-amber-600"><?= number_format($bareme['frais'], 0, ',', ' ') ?> Ar</p>
                                <p class="text-xs text-slate-400">Frais</p>
                            </div>
                            <form action="<?= base_url('bareme-frais/delete/' . $bareme['id']) ?>" method="POST" onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer cette tranche ?')" class="inline">
    ...
    <div class="modal-overlay" id="modal">
        <div class="modal-content">
            ...
            <form action="<?= base_url('bareme-frais/add') ?>" method="POST">
                <div class="space-y-5">
                    <input type="hidden" name="id_type_operation" id="id_type_operation" value="">
                    <input type="hidden" name="id_operateur" id="id_operateur" value="<?= $operateurs[0]['id'] ?? 1 ?>">
    ...
    <script>
        ...
        // Mettre à jour l'opérateur sélectionné dans le formulaire
        document.getElementById('operateurSelect')?.addEventListener('change', function() {
            document.getElementById('id_operateur').value = this.value;
        });
    </script>
</body>
</html>